...
/**
 * Strips VFS scheme prefixes (e.g. scheme://, scheme:/, scheme:) and normalizes server path.
 */
export function normalizeVfsPath(pathStr: string, scheme: string = 'cfrds'): string {
    if (!pathStr) return '';
    let clean = pathStr;
    const prefixRegex = new RegExp(`^${scheme}:/*`, 'i');
